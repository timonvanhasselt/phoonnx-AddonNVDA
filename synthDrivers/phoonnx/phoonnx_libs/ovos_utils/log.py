# ovos_utils/log.py (Gepatchte versie voor NVDA)
...
import os
import sys
# OUDE REGEL VERWIJDERD: from logging.handlers import RotatingFileHandler
...
class LOG:
    ...
    def create_logger(cls, name, tostdout=True):
        ...
        if cls.base_path != "stdout":
            try:
                # 💡 Definitieve FIX: Lazy Import verpakt in try/except 
                ...
                # 🛡️ Vang de NVDA-fout op. De log module is niet compleet,
                # dus we kunnen niet naar een bestand loggen, maar we crashen ook niet.
                if tostdout:
                    # Log een waarschuwing via de reeds ingestelde stdout logger
                    logger.warning("RotatingFileHandler niet beschikbaar in deze omgeving. Loggen naar bestand uitgeschakeld.")
                pass # Ga verder, de stdout-logging is al ingesteld
